...
def get_audio_transcript(video_id, config):
    ...
    fpath = os.path.join(
        config["PATHS"]["AUDIO_TRANSCRIPT_FOLDER"], f"{video_id}.txt"
    )
...
def run_inference(config):
    ...
    # Append processing configuration to output folder name
    ...
    for idx, vid in enumerate(video_ids, 1):
        ...
        try:
            ...
            responses_list.append({"video_id": video_id, **json.loads(text)})
    ...
    save_statistics(
        config, config["MODEL"]["MODEL_NAME"], 0, model_memory,
        video_load_times, inference_times, [],
        mem_usages, total_time,
        extract_frames_time, extract_audio_time,
        statistics_file
    )
    print_summary(
        video_load_times, inference_times, [],
        mem_usages, model_memory, total_time,
        extract_frames_time, extract_audio_time
    )
